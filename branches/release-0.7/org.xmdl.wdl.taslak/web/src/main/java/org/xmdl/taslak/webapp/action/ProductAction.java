...
public class ProductAction extends BaseAction implements Preparable {
	...
	public String list() {
		...
		if (log.isDebugEnabled())
			log.debug("listing items:" + products == null ? null : products
					.size());
		if (log.isDebugEnabled())
			log.debug("list() ->");
		return SUCCESS;
	...
	public String copy() {
		...
		if (log.isDebugEnabled())
			log.debug("copying product: " + product);
		if (log.isDebugEnabled())
			log.debug("copy() ->");
		...
		if (getDeleteId() != null) {
			for (String idStr : getDeleteId()) {
				try {
					...
					if (log.isDebugEnabled())
						log.debug("deleting product with id: " + idStr);
				} catch (DataIntegrityViolationException e) {
					e.printStackTrace();
					cannotDeleted = true;

					if (log.isDebugEnabled())
						log.debug("could not delete product with id: " + idStr);
				}
			}
		}
		if (cannotDeleted)
			saveMessage(getText("Product.cannotBeDeleted"));
		if (anyDeleted)
			saveMessage(getText("Product.deleted"));
	...
	/*PROTECTED REGION ID(Product.act.bdy) ENABLED START*/
