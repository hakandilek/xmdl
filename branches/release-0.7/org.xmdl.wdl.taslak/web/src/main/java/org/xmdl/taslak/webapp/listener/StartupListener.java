...
public class StartupListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		Map<String, Object> config = (HashMap<String, Object>) context
				.getAttribute(BaseConstants.CONFIG);
		...
		if (context.getInitParameter(BaseConstants.CSS_THEME) != null) {
			config.put(BaseConstants.CSS_THEME, context
					.getInitParameter(BaseConstants.CSS_THEME));
		}
		...
		if (log.isDebugEnabled()) {
			...
			if (encryptPassword) {
				log.debug("Encryption Algorithm: "
						+ config.get(ApplicationConstants.ENC_ALGORITHM));
			}
	...
	public static void setupContext(ServletContext context) {
		...
		context.setAttribute(ApplicationConstants.AVAILABLE_ROLES, mgr
				.getAllRoles());
	...
	/*PROTECTED REGION ID(taslak.stdl.bdy) ENABLED START*/
